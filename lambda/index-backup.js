...
const AUTH_TOKEN = process.env.LIMITLESS_AUTH_TOKEN; // Set this in Lambda environment variables

/**
 * Lambda handler that fetches data from Limitless Exchange API and uploads to S3
 * Triggered periodically via EventBridge (e.g., every 1-5 minutes)
 */
 ...
 * API Docs: https://api.limitless.exchange/api-v1#tag/portfolio/get/portfolio/positions
 */
async function fetchPositions() {
  console.log('Fetching positions from Limitless API...');

  try {
    const response = await fetch(`${LIMITLESS_API_URL}/portfolio/positions`, {
      method: 'GET',
      headers: {
    ...
    if (!response.ok) {
      throw new Error(`Positions API request failed: ${response.status} ${response.statusText}`);
    }
 ...
 * API Docs: https://api.limitless.exchange/api-v1#tag/portfolio/get/portfolio/trades
 */
async function fetchTrades() {
  console.log('Fetching trades from Limitless API...');

  try {
    // Fetch all trades (you can adjust limit or paginate if needed)
    const response = await fetch(`${LIMITLESS_API_URL}/portfolio/trades?page=1&limit=100`, {
      method: 'GET',
      headers: {
        'Authorization': AUTH_TOKEN,
        'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      throw new Error(`Trades API request failed: ${response.status} ${response.statusText}`);
    }
 ...
 * API Docs: https://api.limitless.exchange/api-v1#tag/portfolio
 */
async function fetchPoints() {
  console.log('Fetching points from Limitless API...');

  try {
    const response = await fetch(`${LIMITLESS_API_URL}/portfolio/points`, {
      method: 'GET',
      headers: {
        'Authorization': AUTH_TOKEN,
        'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      console.log('Points endpoint not available or returned error');
      return null;
    }
 ...
 * API Docs: https://api.limitless.exchange/api-v1#tag/public-portfolio/get/portfolio/{account}/traded-volume
 * Note: This requires account address - will be derived from authenticated user
 */
async function fetchTradedVolume() {
  ...
  try {
    ...
    // This might be in the points response or we may need a separate endpoint
    // For now, trying the general endpoint
    const response = await fetch(`${LIMITLESS_API_URL}/portfolio/traded-volume`, {
      method: 'GET',
      headers: {
        'Authorization': AUTH_TOKEN,
        'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      console.log('Traded volume endpoint not available via authenticated route');
      return null;
    }
...
function formatState(positions) {
  ...
  // Group positions by wallet (if multiple wallets exist)
  const state = {
    wallet1: {
...
function calculateStats(trades, points, volume) {
  ...
  // Filter completed trades (SELL or REDEEM)
  ...
  // Calculate uptime from first trade
